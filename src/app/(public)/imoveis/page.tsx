...
export default async function ImoveisPage({ searchParams }: ImoveisPageProps) {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-4 py-10">
      <h1 className="mb-6 text-2xl font-bold text-ink">Imóveis disponíveis</h1>
      ...
      {totalPages > 1 && (
        <nav className="mt-8 flex items-center justify-center gap-2 text-sm">
          {page > 1 && (
            <Link href={pageHref(page - 1)} className="rounded-full border border-border px-4 py-2 hover:bg-white">
              Anterior
            </Link>
          )}
